...
**Frames 0 and 382 are keyframes.** The last one is what makes skipping cheap: the
player clears the page, seeks to frame 382 and applies one delta, rather than
replaying 382 of them. No other keyframes are needed — the stream is otherwise
...
**Ruling, 2026-08-04: the opening plays once per boot.** `Engine::run` re-invokes
`runTitle` whenever the player picks RETURN TO MENU from the pause screen, so an
unconditional call would replay the whole opening — and its disc stall — on every
return to the title. A guard inside the player suppresses every call after the
first. It is set before any disc access, so a missing or malformed file cannot
cause a fresh read attempt on each return either.
...
**Correction, 2026-08-04.** This spec originally asserted that the GIF's last frame
is the wordmark card, and cited an RMS of 1.7 as evidence it quantises cleanly. Both
were wrong. The GIF **fades to black over its final fifteen frames** to loop back to
its black opening: ink measures ~2220 samples at frame 382 and 13 by frame 395. The
RMS figure was measured against that near-black frame, where any palette scores well,
so it never supported the claim.

**Playback therefore ends at frame 382**, the last frame within 3% of peak ink, and
frames 383–397 are not encoded at all. Run time is 15.32 s over 383 frames. The
alternatives — replaying the fade and then bringing the card back, or leaving the
menu on black — were rejected as reading like a glitch and as discarding the artwork
respectively.

The frame 382 content is the wordmark plus three copyright lines on black — the
...
**`MENU_ART_TITLE_BACKDROP`** — frame 382 at 320×200 4bpp, quantised onto
